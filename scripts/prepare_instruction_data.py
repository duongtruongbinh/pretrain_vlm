...
def main() -> None:
    config = load_config("instruction_data")
    dataset_name = str(config["dataset_name"]).strip()
    output_dir = Path(config["output_dir"]).expanduser().resolve()
    images_root = output_dir / "images"

    log(f"[instruction-data] loading dataset '{dataset_name}'...")
    ...
    preview_row = dataset_dict[first_split][0]
    preview_summary = {
    ...
    try:
        ...
        for raw_split in raw_splits:
            ...
            log(f"[instruction-data] processing raw split '{raw_split}' with {len(raw_dataset)} rows...")
